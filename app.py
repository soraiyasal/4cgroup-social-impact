...
def get_fiscal_years(data):
    """Get list of available fiscal years from data"""
    if data.empty:
        return []
    
    # Get the first and last dates in the dataset
    min_date = data['Activity Date'].min()
    max_date = data['Activity Date'].max()
    
    fiscal_years = []
    
    # Calculate the range of fiscal years in the data
    min_fy_year = min_date.year - 1 if min_date.month < 4 else min_date.year
    max_fy_year = max_date.year if max_date.month >= 4 else max_date.year - 1
    
    # Create a list of fiscal years
    for year in range(min_fy_year, max_fy_year + 1):
        fiscal_years.append(f"FY{year}/{year+1}")
    
    return fiscal_years

def filter_data_by_period(data, period_selection, latest_date):
    """Filter data based on selected period"""
    if data.empty:
        return data, "No data available"
    
    if period_selection == "Latest Month":
        filtered_data = data[
            (data['Activity Date'].dt.month == latest_date.month) & 
            (data['Activity Date'].dt.year == latest_date.year)
        ]
        period_text = latest_date.strftime("%B %Y")
    elif period_selection == "Financial YTD":
        ...
        # Handle specific fiscal year selections (e.g., "FY2023/2024")
        try:
            fy_years = period_selection.replace("FY", "").split("/")
            start_year = int(fy_years[0])
            end_year = int(fy_years[1])
            
            fy_start = datetime(start_year, 4, 1)
            fy_end = datetime(end_year, 3, 31)
            
            filtered_data = data[
                (data['Activity Date'] >= fy_start) & 
                (data['Activity Date'] <= fy_end)
            ]
            period_text = f"{period_selection} (Apr-Mar)"
        except:
            # Fallback to all data if parsing fails
            filtered_data = data
            period_text = "All Time"
    
    return filtered_data, period_text
...
def show_dashboard(data):
    ...
    fiscal_years = get_fiscal_years(data)
    
    # Time period selector with more useful time periods
    st.markdown('<div class="period-selector">', unsafe_allow_html=True)
    
    # Create dropdown with more practical time period options
    period_options = [
        "Current Month", 
        "Last Month",
        "Last 3 Months",
        "Financial YTD",
        "Last Financial Year"
    ] + [fy for fy in fiscal_years if fy != "Financial YTD" and "Last" not in fy]
    ...
    # Filter data based on selected period
    filtered_data, period_text = filter_data_by_period(data, view_type, latest_date)
    ...
    # Add period text under header
    st.markdown(f"<p style='text-align: center; color: #64748b; margin-bottom: 20px;'>Showing data for: <b>{period_text}</b></p>", unsafe_allow_html=True)
    ...
    for idx, charity in charity_metrics.iterrows():
        ...
        st.markdown(f"""
            <div class="charity-card">
                <div class="charity-header">
                    <div class="charity-logo" style="background-color: {color};">
                        {charity['Organization'][0].upper() if len(charity['Organization']) > 0 else '?'}
                    </div>
                    <div class="charity-name" title="{charity['Organization']}">
                        {charity['Organization']}
                    </div>
    ...
    if not sdg_metrics.empty:
        ...
        for idx, sdg in sdg_metrics.iterrows():
            ...
            # Skip if SDG name is None or NaN
            if pd.isna(sdg_name) or sdg_name is None:
                continue
